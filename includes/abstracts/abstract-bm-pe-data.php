...
abstract class BM_PE_Data {
	...
	public function save() {
		if ( $this->data_store ) {
			...
			do_action( 'press_events_before_' . $this->object_type . '_object_save', $this, $this->data_store );
	...
	protected function is_internal_meta_key( $key ) {
		...
		_doing_it_wrong( __FUNCTION__, sprintf( __( 'Generic add/update/get meta methods should not be used for internal meta data, including "%s". Use getters and setters.', 'press-events' ), $key ), '3.2.0' );
	...
	protected function get_hook_prefix() {
		return 'press_events_' . $this->object_type . '_get_';
	}
	 ...
	 */
	protected function set_date_prop( $prop, $value, $event_id = false ) {
		try {
			if ( empty( $value ) ) {
			...
			if ( $timezone_set ) {
				...
				$datetime->set_utc_offset( $timezone_offset );
			}
