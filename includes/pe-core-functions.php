...
function pe_get_template_part( $slug, $name = '' ) {
	...
	if ( $name && ! PE_TEMPLATE_DEBUG_MODE ) {
		$template = locate_template( array( "{$slug}-{$name}.php", pe()->template_path() . "{$slug}-{$name}.php" ) );
	}

	// Get default slug-name.php
	if ( ! $template && $name && file_exists( PE()->plugin_path() . "/templates/{$slug}-{$name}.php" ) ) {
		$template = PE()->plugin_path() . "/templates/{$slug}-{$name}.php";
	}
	...
	if ( ! $template && ! PE_TEMPLATE_DEBUG_MODE ) {
		$template = locate_template( array( "{$slug}.php", pe()->template_path() . "{$slug}.php" ) );
	}
...
function pe_locate_template( $template_name, $template_path = '', $default_path = '' ) {
	if ( ! $template_path ) {
		$template_path = PE()->template_path();
	}

	if ( ! $default_path ) {
		$default_path = PE()->plugin_path() . '/templates/';
	}
