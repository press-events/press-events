...
class BM_PE_HookFinder {
	...
	private static function get_hook_link( $hook, $details = array() ) {
		if ( ! empty( $details['class'] ) ) {
			$link = 'http://docs.woocommerce.com/pe-apidocs/source-class-' . $details['class'] . '.html#' . $details['line'];
		} elseif ( ! empty( $details['function'] ) ) {
			$link = 'http://docs.woocommerce.com/pe-apidocs/source-function-' . $details['function'] . '.html#' . $details['line'];
		} else {
	...
	public static function process_hooks() {
		...
		$template_files[]	= '../includes/pe-template-functions.php';
		$template_files[]	= '../includes/pe-template-hooks.php';
		...
		foreach ( self::$files_to_scan as $heading => $files ) {
			...
			foreach ( self::$custom_hooks_found as $hook => $details ) {
				if ( ! strstr( $hook, 'press_events' ) && ! strstr( $hook, 'event' ) && ! strstr( $hook, 'pe_' ) ) {
		...
		$html   = file_get_contents( '../pe-apidocs/tree.html' );
		...
		file_put_contents( '../pe-apidocs/hook-docs.html', $header . ob_get_clean() . end( $footer ) );
